...
void QQConv2D_Optimized_PReLU(int out[], 
                   int out_ch, 
                   int out_dim, 
                   int in_ch, 
                   int in_dim, 
                   ...
                   uint8_t input[])
{
    ...
    int next_filter = in_ch * ker_dim * ker_dim;
        ...
        for (j = 0; j < out_dim; j++)
        {
            for (k = 0; k < out_dim; k++)
            {
                ...
                for(int p = 0; p < P; p++)
                {
                    ...
                    if(sum[p] <= 0)
					{
                		conv_out = sum[p] * shift[LU_factor+p];
					}
                    /* Batch Normalization Fusion*/
                    conv_out = (conv_out - bn_alpha1[LU_factor+p]) * bn_alpha2[LU_factor+p];
