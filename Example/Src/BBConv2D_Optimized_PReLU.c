...
void BBConv2D_Optimized_PReLU(int out[], 
               int out_ch, 
               int out_dim, 
               int in_ch, 
               int in_dim, 
                ...
                for (m = 0; m < ker_size; m++)
                {
                    for (n = 0; n < ker_size; n++)
                    {
                        ...
                        if (in_row >= 0 && in_col >= 0 && in_row < in_dim && in_col < in_dim)
                        {
                            ...
                            {
                                weight_idx = (m * ker_size + n) * in_ch_pad>>5 + l;
                            	input_idx = (in_row * in_dim + in_col) * in_ch_pad>>5 + l;

                                sum[0] = __builtin_popcount(weight[filters[0] + weight_idx]^input[input_idx]);
                                pop_count[0] += N - (sum[0]<<1);

                                sum[1] = __builtin_popcount(weight[filters[1] + weight_idx]^input[input_idx]);
                                pop_count[1] += N - (sum[1]<<1);

                                sum[2] = __builtin_popcount(weight[filters[2] + weight_idx]^input[input_idx]);
                                pop_count[2] += N - (sum[2]<<1);

                                sum[3] = __builtin_popcount(weight[filters[3] + weight_idx]^input[input_idx]);
                                pop_count[3] += N - (sum[3]<<1);

                                sum[4] = __builtin_popcount(weight[filters[4] + weight_idx]^input[input_idx]);
                                pop_count[4] += N - (sum[4]<<1);

                                sum[5] = __builtin_popcount(weight[filters[5] + weight_idx]^input[input_idx]);
                                pop_count[5] += N - (sum[5]<<1);

                                sum[6] = __builtin_popcount(weight[filters[6] + weight_idx]^input[input_idx]);
                                pop_count[6] += N - (sum[6]<<1);

                                sum[7] = __builtin_popcount(weight[filters[7] + weight_idx]^input[input_idx]);
                                pop_count[7] += N - (sum[7]<<1);

                                sum[8] = __builtin_popcount(weight[filters[8] + weight_idx]^input[input_idx]);
                                pop_count[8] += N - (sum[8]<<1);

                                sum[9] = __builtin_popcount(weight[filters[9] + weight_idx]^input[input_idx]);
                                pop_count[9] += N - (sum[9]<<1);

                                sum[10] = __builtin_popcount(weight[filters[10] + weight_idx]^input[input_idx]);
                                pop_count[10] += N - (sum[10]<<1);

                                sum[11] = __builtin_popcount(weight[filters[11] + weight_idx]^input[input_idx]);
                                pop_count[11] += N - (sum[11]<<1);

                                sum[12] = __builtin_popcount(weight[filters[12] + weight_idx]^input[input_idx]);
                                pop_count[12] += N - (sum[12]<<1);

                                sum[13] = __builtin_popcount(weight[filters[13] + weight_idx]^input[input_idx]);
                                pop_count[13] += N - (sum[13]<<1);

                                sum[14] = __builtin_popcount(weight[filters[14] + weight_idx]^input[input_idx]);
                                pop_count[14] += N - (sum[14]<<1);

                                sum[15] = __builtin_popcount(weight[filters[15] + weight_idx]^input[input_idx]);
                                pop_count[15] += N - (sum[15]<<1);

                                sum[16] = __builtin_popcount(weight[filters[16] + weight_idx]^input[input_idx]);
                                pop_count[16] += N - (sum[16]<<1);

                                sum[17] = __builtin_popcount(weight[filters[17] + weight_idx]^input[input_idx]);
                                pop_count[17] += N - (sum[17]<<1);

                                sum[18] = __builtin_popcount(weight[filters[18] + weight_idx]^input[input_idx]);
                                pop_count[18] += N - (sum[18]<<1);

                                sum[19] = __builtin_popcount(weight[filters[19] + weight_idx]^input[input_idx]);
                                pop_count[19] += N - (sum[19]<<1);

                                sum[20] = __builtin_popcount(weight[filters[20] + weight_idx]^input[input_idx]);
                                pop_count[20] += N - (sum[20]<<1);

                                sum[21] = __builtin_popcount(weight[filters[21] + weight_idx]^input[input_idx]);
                                pop_count[21] += N - (sum[21]<<1);

                                sum[22] = __builtin_popcount(weight[filters[22] + weight_idx]^input[input_idx]);
                                pop_count[22] += N - (sum[22]<<1);

                                sum[23] = __builtin_popcount(weight[filters[23] + weight_idx]^input[input_idx]);
                                pop_count[23] += N - (sum[23]<<1);

                                sum[24] = __builtin_popcount(weight[filters[24] + weight_idx]^input[input_idx]);
                                pop_count[24] += N - (sum[24]<<1);

                                sum[25] = __builtin_popcount(weight[filters[25] + weight_idx]^input[input_idx]);
                                pop_count[25] += N - (sum[25]<<1);

                                sum[26] = __builtin_popcount(weight[filters[26] + weight_idx]^input[input_idx]);
                                pop_count[26] += N - (sum[26]<<1);

                                sum[27] = __builtin_popcount(weight[filters[27] + weight_idx]^input[input_idx]);
                                pop_count[27] += N - (sum[27]<<1);

                                sum[28] = __builtin_popcount(weight[filters[28] + weight_idx]^input[input_idx]);
                                pop_count[28] += N - (sum[28]<<1);

                                sum[29] = __builtin_popcount(weight[filters[29] + weight_idx]^input[input_idx]);
                                pop_count[29] += N - (sum[29]<<1);

                                sum[30] = __builtin_popcount(weight[filters[30] + weight_idx]^input[input_idx]);
                                pop_count[30] += N - (sum[30]<<1);

                                sum[31] = __builtin_popcount(weight[filters[31] + weight_idx]^input[input_idx]);
